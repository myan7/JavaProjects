...
1 <= x, y, ai, bi <= 10^4
 */
public class EASY_1779_FindNearestPointThatHastheSameXorYCoordinate {
    /*
    Runtime: 1 ms, faster than 100.00% of Java online submissions for Find Nearest Point That Has the Same X or Y Coordinate.
    Memory Usage: 49.9 MB, less than 85.55% of Java online submissions for Find Nearest Point That Has the Same X or Y Coordinate.
     */
    public int nearestValidPoint(int x, int y, int[][] points) {
        ...
        for(int i = 0 ; i < points.length; i++)
        {
            ...
            if(currX == x || currY == y)
            {
                int currDist = Math.abs(currX - x) + Math.abs(currY - y);
                if(currDist < dist)
                {
                    ans = i;
                    dist = currDist;
