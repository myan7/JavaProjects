package com.myjobhunting;
// https://leetcode.com/problems/smallest-string-with-a-given-numeric-value/

import java.util.Arrays;

/*
The numeric value of a lowercase character is defined as its position (1-indexed) in the alphabet,
so the numeric value of a is 1, the numeric value of b is 2, the numeric value of c is 3, and so on.

The numeric value of a string consisting of lowercase characters is defined as the sum of its characters' numeric values.
For example, the numeric value of the string "abe" is equal to 1 + 2 + 5 = 8.

You are given two integers n and k.
Return the lexicographically smallest string with length equal to n and numeric value equal to k.

Note that a string x is lexicographically smaller than string y if x comes before y in dictionary order,
that is, either x is a prefix of y, or if i is the first position such that x[i] != y[i], then x[i] comes before y[i] in alphabetic order.

Example 1:
Input: n = 3, k = 27
Output: "aay"
Explanation: The numeric value of the string is 1 + 1 + 25 = 27, and it is the smallest string with such a value and length equal to 3.
Example 2:

Input: n = 5, k = 73
Output: "aaszz"

Constraints:

1 <= n <= 105
n <= k <= 26 * n
 */
public class MEDIUM_1663_SmallestStringWithAGivenNumericValue {

    /* optimized right to left approach
    Runtime: 11 ms, faster than 90.48% of Java online submissions for Smallest String With A Given Numeric Value.
    Memory Usage: 42.1 MB, less than 88.89% of Java online submissions for Smallest String With A Given Numeric Value.
     */
    public String getSmallestString(int n, int k)
    {
        char[] tmp = new char[n];
        for(int i = n-1; i>=0 && k > 0; i--)
        {
            int safe2Add = Math.min(k-i,26);
            tmp[i] =(char)(Math.min(safe2Add,26) + 'a'-1);
            k -= safe2Add;
        }
        return new String(tmp);
    }

    /* right to left approach.
    Runtime: 19 ms, faster than 88.36% of Java online submissions for Smallest String With A Given Numeric Value.
    Memory Usage: 64.3 MB, less than 33.33% of Java online submissions for Smallest String With A Given Numeric Value.
     */
    public String getSmallestString0(int n, int k) {
        char[] result = new char[n];
        Arrays.fill(result, 'a');
        k -= n;
        for (int position = n - 1; position >= 0 && k > 0; position--) {
            int add = Math.min(k, 25);
            result[position] = (char) (result[position] + add);
            k -= add;
        }
        return new String(result);
    }
}
